    ...
    const handleStatusUpdate = async (shopId, statusId) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (onUpdate) onUpdate(); // 親（AdminTop）の件数バッジを更新
            }
                ...
                pendingShops.map((shop) => {
                    ...
                    return (
                        <div key={shop.id} style={{
                            ...cardStyle,
                            //borderLeft: isResubmitted ? "10px solid #faad14" : "10px solid #",
                            ...
                            <div onClick={() => setExpandedId(expandedId === shop.id ? null : shop.id)} style={cardHeaderStyle}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                    ...
                                    {isResubmitted ? (
                                        ...
                                        <span style={newBadgeStyle}>新規申請</span>
                                    )}
                                </div>
                            ...
                            <p style={cardSubTextStyle}>
                                申請者ID: {shop.user_id} | 申請日時: {new Date(shop.created_at).toLocaleString()}
                            </p>
                            ...
                            {expandedId === shop.id && (
                                <div style={cardDetailStyle}>
                                    ...
                                    <div style={infoGridStyle}>
                                        <p><strong>見出し:</strong> {shop.catchphrase}</p>
                                        ...
                                        <p><strong>URL:</strong> <a href={shop.url} target="_blank" rel="noreferrer">{shop.url}</a></p>
            ...
            {shops.length === 0 ? (
                ...
                shops.map(shop => {
                    ...
                    return (
                        <div key={shop.id} style={{
                            ...cardStyle,
                            borderLeft: isResubmitted ? "10px solid #" : "10px solid #",
                        }}>
                            <div onClick={() => setExpandedId(expandedId === shop.id ? null : shop.id)} style={cardHeaderStyle}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                ...
                                <span style={{ float: 'right', color: '#', fontSize: '14px', fontWeight: 'normal' }}>
                                    {expandedId === shop.id ? "▲ 閉じる" : "▼ 詳細編集・公開設定"}
                                </span>
                            </div>
                            <p style={cardSubTextStyle}>
                                住所: {shop.address} | 地域: {shop.area?.name || "未設定"}
                            </p>

                            {expandedId === shop.id && (
                                <div style={cardDetailStyle}>
                                    {/* 非公開（status: 9）かつ理由がある場合のみ表示 */}
                                    {status === 9 && shop.rejection_reason && (
                                        <div style={rejectionBoxStyle}>
                                            <strong>⚠️ 備考/却下理由:</strong> {shop.rejection_reason}
                                        </div>
                                    )}
                                    ...
                                    <div style={infoGridStyle}>
                                        <p><strong>見出し:</strong> {shop.catchphrase}</p>
                                        <p><strong>位置情報:</strong> 緯度: {shop.latitude} / 経度: {shop.longitude}</p>
                                        <p><strong>ジャンル:</strong> {shop.genre?.name || "未設定"}</p>
                                        <p><strong>予算:</strong> {shop.budget?.name || "未設定"}</p>
                                        <p><strong>電話番号:</strong> {shop.tel}</p>
                                        <p><strong>営業時間:</strong> {shop.business_hours}</p>
